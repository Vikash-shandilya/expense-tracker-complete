window.addEventListener("DOMContentLoaded", async () => {
  ...
  await showlist(1);
  let check = await axios.get("http://13.48.71.100:3000/ifpremium");
  if (check.data) {
    ...
    let result = await axios.get(
      "http://13.48.71.100:3000/getpreviousdownloads"
    );
    console.log(result);
...
async function showlist(current_page) {
  ...
  let res = await axios.post(`http://13.48.71.100:3000/showexpense/`, myobj);
  console.log(res);
...
async function addexpense() {
  ...
  let res = await axios.post("http://13.48.71.100:3000/expense", myobj);
...
async function deleted(e) {
  ...
  console.log(id);
  let res = await axios.get(`http://13.48.71.100:3000/expenses/deleted/${id}`);
  window.location.reload();
}
...
async function premium() {
  let res = await axios.get("http://13.48.71.100:3000/get_order_details");
  ...
  let options = {
    ...
    handler: async (response) => {
      let res = await axios.post("http://13.48.71.100:3000/updateorder", {
        order_id: orderdetails.id,
  ...
  rzp.on("payment.failed", async function (response) {
    ...
    await axios.post("http://13.48.71.100:3000/paymentfailed", {
      order_id: order_id,
...
async function showleaderboard() {
  let res = await axios.get("http://13.48.71.100:3000/showleaderboard");
  console.log(res);
...
async function downloadreport() {
  let res = await axios.get("http://13.48.71.100:3000/download");
  const createlink = document.createElement("a");
...
async function choosemonth() {
  const ifpremium = await axios.get("http://13.48.71.100:3000/ifpremium");
  console.log(ifpremium.data);
  if (ifpremium.data) {
    const monthname = document.getElementById("month").value;
    console.log(monthname);
    const response = await axios.get(
      `http://13.48.71.100:3000/monthlyexpense/${monthname}`
    );
    let week1 = [];
    let week2 = [];
    let week3 = [];
